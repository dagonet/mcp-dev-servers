...
### Added
- `template-sync-tools`: **lossy-merge safety net on `three_way`**. After every merge the result is checked against base: any line present in `base`, untouched by the project, and kept by the template that occurs fewer times in the merged output (counted, not set-compared) is reported in a new `dropped_lines` list, flips `has_conflicts`/`conflict_count`, and appends a synthetic `<<<<<<< PROJECT (dropped by merge)` hunk — so a consumer can never apply a lossy merge as clean. For merged `.sh` content the tool additionally runs `bash -n`, plus `node --check` on every embedded `node -e '…'` block; a failure surfaces as `syntax_error` with the same conflict treatment (`syntax_checked` reports whether the check ran; missing `bash`/`node` is a graceful skip, never a failure). Motivated by a "clean" merge that lost the closing `];` of a hook's `allowPatterns` array — hooks fail open, so the broken hook silently disabled enforcement. An insertion whose anchor line the other side replaced is likewise surfaced as an `insertion anchor lost` conflict rather than being relocated to the end of the file.
